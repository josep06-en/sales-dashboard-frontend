              ...
              {topProducts.length > 1 && (
                <InsightCard
                  ...
                  description={`Top 3 products account for ${formatPercentage(
                    (topProducts.slice(0, 3).reduce((sum, p) => sum + (p.revenue || 0), 0) / 
                     topProducts.reduce((sum, p) => sum + (p.revenue || 0), 0)) * 100
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <InsightCard
                ...
                description={`Total revenue of ${formatCurrency(kpiOverview.total_revenue || 0)} from ${formatNumber(kpiOverview.total_orders || 0)} orders with average order value of ${formatCurrency(kpiOverview.avg_aov || 0)}.`}
                impact="medium"
              />
              <InsightCard
                title="Data Coverage"
                description={`Analytics cover ${formatNumber(kpiOverview.period_count || 0)} data points.`}
                impact="low"
              />
            </div>
          </div>
              ...
              <div className="space-y-4">
                {topProducts.slice(0, 3).map((product, index) => (
                  <InsightCard
                    ...
                    title={`#${index + 1} Product: ${product.product || 'Unknown'}`}
                    description={`${formatCurrency(product.revenue || 0)} revenue with average price of ${formatCurrency(product.avg_price || 0)} in ${product.category || 'Unknown'} category.`}
